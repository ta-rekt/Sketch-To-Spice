...
int main( int argc, char** argv )
{
  ...
  blur( src_flat, src_flat, Size(12,12) );
...
void thresh_callback(int, void* )
{
  ...
  threshold( src_flat, threshold_output, thresh, 255, THRESH_BINARY );
  ...
  for( size_t i = 0; i < contours.size(); i++ )
  {
    ...
    if(norm(boundRect[i].tl() - boundRect[i].br()) > 50 && norm(boundRect[i].tl() - boundRect[i].br()) < 150)      //magic number
    {
      rectangle( drawing, boundRect[i].tl(), boundRect[i].br(), color, 2, 8, 0 );
      output = src(boundRect[i]);
